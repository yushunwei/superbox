...
数据库完整表结构文档在 `db/schema.md`，包含所有模块的表名、字段名、类型、约束、索引和用途说明。

**加载策略：** 为节省上下文，默认不加载此文件。当需要操作数据库时（编写 SQL、创建表、修改字段、MyBatis 映射等），主动读取 `db/schema.md` 了解当前表结构。新增或修改表结构后，同步更新该文件。
